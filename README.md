...
`@Provider` generates the following code:
...
class AppDependencies {
    ...
    func registerAll(in store: inout InjectionStore) {
        ...
        store.register(userUseCase, for: (any UserUseCaseProtocol).self)
    }
}
...
```swift
struct UseCounter {
    // stored var → computed property delegating to Storage
    var count: Int {
        get { hookStorage.count }
        nonmutating set { hookStorage.count = newValue }
    }

    func increment() { count += 1 }  // works thanks to nonmutating set
